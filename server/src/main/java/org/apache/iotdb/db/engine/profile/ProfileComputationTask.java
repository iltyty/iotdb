...
public class ProfileComputationTask implements Runnable {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(ProfileComputationTask.class);
  ...
  public void scanOneTsFile(
    ...
    Map<Path, FileSeriesStat> seriesStatMap = new HashMap<>();
    for (Path seriesPath : seriesPaths) {
      FileSeriesStat fileSeriesStat = new FileSeriesStat();
      ...
      for (IChunkReader chunkReader : chunkReaders) {
        ...
        try {
          ...
          seriesStatMap.put(seriesPath, fileSeriesStat);
...
//    LOGGER.info(String.format("Calc time: %dms", end - start));
  ...
  class FileSeriesStat {
    private SeriesStat fileStat = new SeriesStat();
    private List<SeriesStat> chunkStats = new ArrayList<>();
    private List<List<SeriesStat>> pageStats = new ArrayList<>();

    public void addPageSeriesStat(SeriesStat stat) {
      int numChunks = chunkStats.size();
      pageStats.get(numChunks - 1).add(stat);
      chunkStats.get(numChunks - 1).merge(stat);
    }

    public void startNewChunk() {
      chunkStats.add(new SeriesStat());
      pageStats.add(new ArrayList<>());
    }
  ...
  static class RDBMS {
    ...
    public void writeOneTsFileStat(String filePath, List<Path> seriesPaths, Map<Path, FileSeriesStat> seriesStatMap) {
      try (Connection conn = ds.getConnection()) {
        ...
        int fid = writeToFile(writeToFileStmt, filePath, UtilFunc.getFileVersion(filePath));
        writeToSeries(writeToSeriesStmt, seriesPaths);

        for (Map.Entry<Path, FileSeriesStat> entry : seriesStatMap.entrySet()) {
          Path seriesPath = entry.getKey();
          querySeriesStmt.setString(1, seriesPath.getFullPath());
          int sid = querySeriesStmt.executeQuery().getInt(1);

          FileSeriesStat fileSeriesStat = entry.getValue();
          SeriesStat fileStat = fileSeriesStat.fileStat;
          List<SeriesStat> chunkSeriesStats = fileSeriesStat.chunkStats;
          List<List<SeriesStat>> pageSeriesStats = fileSeriesStat.pageStats;

          writeToFileSeriesStat(writeToFileSeriesStmt, fid, sid, fileStat);
          for (int i = 0; i < chunkSeriesStats.size(); i++) {
            SeriesStat chunkStat = chunkSeriesStats.get(i);
            int cid = writeToChunk(writeToChunkStmt, fid, sid, i + 1);
            writeToChunkSeriesStat(writeToChunkSeriesStmt, cid, chunkStat);

            List<SeriesStat> pageStats = pageSeriesStats.get(i);
            for (int j = 0; j < pageStats.size(); j++) {
              SeriesStat pageStat = pageStats.get(j);
              int pid = writeToPage(writeToPageStmt, cid, sid, j + 1);
        ...
        logger.error("Error writing one tsfile statistic to sqlite: ", e);
      }
    }
